...
def test_extract_movie_details_standard_format():
    cleaner = MovieFilenameCleaner(
        directory="Media/Movies", filename="The Dark Knight (2008).mkv"
    )
...
def test_extract_movie_details_dot_format():
    cleaner = MovieFilenameCleaner(
        directory="Media/Movies", filename="The.Dark.Knight.2008.1080p.mkv"
    )
    cleaner.filename_working = "The Dark Knight 2008"
    result = cleaner.extract_movie_details()
    assert result["title"] == "The Dark Knight"
...
def test_extract_movie_details_square_brackets():
    cleaner = MovieFilenameCleaner(directory="Media/Movies", filename="Test.mkv")
    cleaner.filename_working = "The Dark Knight [2008]"
    result = cleaner.extract_movie_details()
    assert result["title"] == "The Dark Knight"
    assert result["year"] == "2008"
...
def test_extract_movie_details_no_details():
    cleaner = MovieFilenameCleaner(directory="Media/Movies", filename=".mkv")
    cleaner.filename_working = ".mkv"
    result = cleaner.extract_movie_details()
    assert result["title"] is None
    assert result["year"] is None
